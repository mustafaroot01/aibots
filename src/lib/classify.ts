...
/** كلمات تدل إن الجهة تدوّر على موظف — بالفصحى واللهجة العراقية */
const JOB_WORDS = [
  // فصحى
  "مطلوب", "مطلوبين", "نحتاج", "يحتاج", "تحتاج", "بحاجة", "بحاجه",
  "تعيين", "تعيينات", "شاغر", "شواغر", "فرصة عمل", "فرص عمل", "توظيف",
  "وظيفة", "وظائف", "كادر", "للعمل", "يرجى التقديم", "التقديم", "نبحث عن",
  "تعلن عن حاجتها", "يعلن عن حاجته", "استقدام", "انضم لفريق", "انضم الى فريق",
  // لهجة عراقية
  "محتاجين", "محتاجه", "نريد", "نرغب", "دنحتاج", "عدنا شاغر", "عندنا شاغر",
  "يريد موظف", "نريد موظف", "تريد موظف", "للتعيين", "مطلوبه", "مطلوبة",
];

/** أنماط أوسع من الكلمات المفردة — تمسك الصيغ اللي تنكتب بترتيب مختلف */
const JOB_PATTERNS = [
  /(يعلن|تعلن|نعلن)[^\n]{0,40}(حاجت|رغبت|شاغر|توظيف|تعيين|فرص)/,
  /(بحاجة|بحاجه)\s*(ال[ىي]|إل[ىي]|ل)/,
  /(نبحث|يبحث|تبحث)\s*عن\s*(موظف|كادر|شخص|شباب|بنات|فتيات|عامل|سائق|مندوب)/,
  /(محتاج|محتاجين|نريد|مطلوب)\s*\S{0,12}\s*(موظف|عامل|كادر|سائق|مندوب|بنت|بنات|شباب|فتيات|كاشير|محاسب)/,
  /(فرصة|فرص)\s*(عمل|توظيف|وظيفية)/,
];

/** أنماط رفض — صاحب المنشور هو اللي يدوّر على شغل */
const REJECT_PATTERNS = [
  /(ادور|أدور|ابحث|أبحث|ابغى|اريد|أريد|محتاج)\s*(على|عن)?\s*(عمل|شغل|وظيفة|وظيفه)\b/,
  /(خريج|خريجة|متخرج)[^\n]{0,30}(ادور|أدور|ابحث|أبحث|اريد|أريد)/,
];

/** كلمات تدل إن صاحب المنشور هو اللي يدوّر على شغل — أو إعلان مو وظيفة */
const REJECT_WORDS = [
  // شخص يدوّر على عمل
  "ادور على عمل", "أدور على عمل", "ادور على شغل", "أدور على شغل",
  "ابحث عن عمل", "أبحث عن عمل", "ابحث عن وظيفة", "أبحث عن وظيفة",
  "خريج وابحث", "محتاج شغل", "محتاج عمل", "اريد عمل", "أريد عمل",
  "اريد شغل", "أريد شغل", "ابغى عمل", "دور لي على شغل", "دورولي على شغل",
  // إعلانات مو وظائف
  "للايجار", "للإيجار", "للبيع", "دورة", "كورس", "اشترك", "استثمار",
  "ربح", "ارباح", "أرباح",
];
...
export function rulesTitle(raw: string): string {
  ...
  const line = lines.find((l) => JOB_WORDS.some((w) => l.includes(w)) || JOB_PATTERNS.some((re) => re.test(l)))
    ?? lines[0] ?? "";
